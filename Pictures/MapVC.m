...
- (void)detailButtonTapped
{
    ...
    dispatch_async(loadingQueue, ^{
        UIImage *image = [[Cacher sharedInstance] getImageForPhotoId:[self.currentPhoto objectForKey:FLICKR_PHOTO_ID]];
        if (!image) {
            NSLog(@"loading image");
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([segue.identifier isEqualToString:@"showImageFromAnnotation"]) {
        ...
        dvc.title = [self.currentPhoto objectForKey:FLICKR_PHOTO_TITLE];
    }
}
